...
End-to-end test: SPI config → FSM cycling → fault injection → IRQ → ADC.
"""
...
async def test_full_integration(dut):
    """Full integration test: config → run → fault → IRQ → clear."""
    ...
    dut._log.info("Step 2: Read back configuration")
    ...
    # Step 3: Verify analog outputs
    dut._log.info("Step 3: Verify analog config outputs")
    assert int(dut.gain.value) == 2
    assert int(dut.tune1.value) == 5
    assert int(dut.thresh.value) == 0x40

    # Step 4: Run classifier FSM — watch for fsm_done pulses
    dut._log.info("Step 4: Run classifier FSM with normal input")
    ...
    # Step 5: Inject fault
    dut._log.info("Step 5: Inject ball fault (class=3)")
    ...
    # Step 8: ADC test
    dut._log.info("Step 8: ADC control test")
    # Set ADC data BEFORE triggering start (simulates ADC providing data)
    dut.adc_data_in.value = 0xAB
    await spi_write(dut, 0x0D, 0x05)  # channel=1, start=1
    await ClockCycles(dut.clk, 20)  # wait for conversion + done
    ...
    dut._log.info(f"  ADC_DATA = 0x{adc_data:02X}")
...
async def test_fsm_phase_signals(dut):
    """Verify FSM sample/evaluate/compare signals are generated."""
