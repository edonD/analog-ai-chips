...
def discover() -> list[str]:
    """Plans and design netlists worth one click."""
    out: list[str] = []
    for root, dirs, files in os.walk(REPO):
        dirs[:] = [d for d in dirs if not d.startswith((".", "__"))
                   and d != "node_modules"]
        for f in files:
            if f.endswith(".plan") or \
               (f.endswith((".cir", ".spice")) and f.startswith("design")):
                out.append(os.path.relpath(os.path.join(root, f), REPO))
    return sorted(out, key=lambda p: (not p.endswith(".plan"), p))
...
class Launcher:
    def __init__(self) -> None:
        ...
        ttk.Label(mid, text="Discovered (double-click to select; .plan = "
                            "live plan mode):").pack(anchor="w")
    ...
    def browse(self) -> None:
        f = filedialog.askopenfilename(
            initialdir=REPO,
            filetypes=[("netlists & plans", "*.cir *.spice *.sp *.plan"),
                       ("all files", "*.*")])
        if f:
            self.file_var.set(f)
    ...
    def pick(self, _ev) -> None:
        ...
        if sel:
            self.file_var.set(os.path.join(REPO,
                                           self.listbox.get(sel[0])))
