...
| 33 | SpiceGlass — now its own repo at [`../spiceglass`](../spiceglass) | **Open SpiceVision PRO for the AI design era**: deterministic `.cir`→verified-schematic converter + interactive editor. Extracted from this repo (was `spiceglass/`) with full git history. |
...
![VibroSense-1 System Architecture](vibrosense/system_block_diagram.png)
...
| Date | What |
|------|------|
| 2026-06-17 | **SpiceGlass extracted to its own repo.** Moved `spiceglass/` out to `../spiceglass` (parallel folder) via `git subtree split`, preserving all 101 commits. Made the standalone repo self-contained: vendored the 4 golden netlists + 3 symbols under `tests/fixtures/`, repointed its tests/tools (CORE corpus now reads the sibling `../analog-ai-chips`), and merged the SpiceGlass `.gitignore` rules into it. Standalone tests green (4/4). |
| 2026-06-10 | **SpiceGlass M0 SHIPPED.** Deterministic .cir→schematic converter working: branch-column analog placer (rails folded, PMOS-up/NMOS-down stacks, section-comment clustering, rail-distance row anchoring), lane-allocating router, SVG symbols, and round-trip geometric verification. All 8 sheets across 4 real vibrosense netlists render VERIFIED; the verifier caught a real router short during bring-up. Zero dependencies. (Now in the `../spiceglass` repo.) |
